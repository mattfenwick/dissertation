...
our lab, ST \cite{connjur-st} and WB, alleviate the
reproduciblity problem in the area of spectral reconstruction.

ST was designed with the general goal of
...
This enables ST to run without modification on virtually any
...
additional tools do not run there.  WB also allows export of
...
A key component of WB's success is its robust model of spectral
reconstruction, and the presentatio of its model to the user.  WB
...
made publicly available for dissemination.  ST, WB
...
the sample scheduler, the Sparky extension, as well as ST and
WB to a limited extent -- have been purposely designed with 
...
\begin{figure}
  ...
  \caption[WB's data model]
          {WB's data model.  Generated with MySQLWorkbench.}
  \label{wb_model}
\end{figure}

...
algorithms are (see Table-\ref{scheduler_components} for a table):
\begin{itemize}
...
% TODO why not include them all?  might want to break into two tables
\begin{table}[h]
  \begin{tabular}{ | c || c | c | }
    \hline
      Component category    &  Name         &  Explanation                                          \\  \hline  \hline
      Point generator       &  Halton       &  generates a sub-random sequence of points            \\  \hline
      Point generator       &  Poisson gap  &  generates points according to the Poisson gap method \\  \hline
      Point selector        &  Exponential  &  select points with an exponentially-decaying weight  \\  \hline
      Point selector        &  Random       &  select points with a random weight                   \\  \hline
      Quadrature generator  &  All          &  generate all quadrature components for each point    \\  \hline
      Quadrature generator  &  Single random  &  randomly generate a single quadrature component for each point                 \\  \hline
      Forced point selector &  First point  &  force the point with the minimum time-delay in each dimension to be collected    \\  \hline
      Forced point selector &  Last point   &  force the point with the maximum time-delay in each dimension to be collected    \\  \hline
      Formatter             &  Bruker       &  output the schedule in Bruker format     \\  \hline
      Formatter             &  Agilent      &  output the schedule in Agilent format    \\  \hline
      Formatter             &  RNMRTK       &  output the schedule in RNMRTK format     \\  \hline
      Formatter             &  JSON         &  output the schedule in JSON format       \\  \hline
    \hline 
  \end{tabular}
  \caption{Selected components of the sample scheduler.}
  \label{scheduler_components}
\end{table}

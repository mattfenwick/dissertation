...
\section{An overview of the data involved}
...
\subsection{Molecule}
\subsubsection{Known}
The primary sequence of amino acids, as well as the atoms contained in each
amino acid, bond lengths between atom pairs, and bond angles are known.
...
\subsection{Sample}
In protein NMR, the sample is typically carefully prepared so that its contents
and concentration are known before running any NMR experiments.  In other
NMR applications, such as small molecule metabolite identification, the sample
is an unknown.

\subsection{NMR}
\subsubsection{Known}
Information about the spectrometer, including frequency, as well as 
gyromagnetic ratios of nuclei and pulse sequences.  The pulse sequences
determine which covalently bond atom groups will be visible in experiments.
The concept of a resonance -- that the resonance frequency of an atom is 
more or less a constant -- is known.
\subsubsection{Collected data}
The experimental conditions in which data is collected, the sampling schedule,
and that resonances appear as decaying sinusoids is known.  This last is
exploited in transforming time-domain data sets to frequency-domain.
\subsubsection{Processed data}
Spectra contain peaks, which are characterized to obtain volume and peak
dimension attributes; peak dimensions are characterized to obtain position
and widt attributes.
\subsubsection{Unknown}
The correspondence between resonances, which appear in NMR spectra as peak
dimensions, and atoms is not known.  The correspondence between resonances
and peak dimensions is also not known, and is difficult to determine in the
presence of ambiguity.

\subsection{Analysis}
\subsubsection{Known}
This is documented fully in the library of deductive reasoning.
\subsubsection{Unknown}
...
constraints include H-H interatomic distances, torsion angles, and RDCs.



\section{Current data models for analysis}
This section will present a brief outline of the current standard data 
models used in NMR, as well as their limitations and the problems those
limitations cause for reproducibility of analysis.

\subsection{BMRB: NMR-Star data dictionary}
Complete documentation of the NMR-Star data dictionary may be found online 
at \url{http://www.bmrb.wisc.edu/dictionary/}.

\subsubsection{Peak list}
A peak list roughty corresponds to an NMR spectrum.  It includes information
about the spectrum, linking to sample conditions and a pulse sequence, and
also describes the spectral conditions.  It includes a list of peaks as well.

\subsubsection{Peak}
A peak is defined as a feature of a peak list, with a dimension for each 
dimension of the peak list.  Each dimension has a chemical shift value, and
may have additional characterizations.  The peak as a whole has characterizations
including intensity.

\subsubsection{Resonance}
A resonance is used to link a peak dimension to an atom, for a chemical shift
assignment, by means of a spin system and a residue.  Each peak dimension is
...
A spin system is a group of covalently bound resonances.  It may be assigned
to a protein residue.  It may also be assigned an amino acid type.  It is a
key intermediary between NMR peaks and the protein.

\subsubsection{Isotope}
The dimensions of a peak list each target specific isotopes, such as 1H or
13C.  The atoms of molecule must have these specific nuclei in order to 
show up in the experiment.

\subsubsection{Molecule, residue and atom}
These are given standard definitions, compatible with those given by the
CCPN data model.

\subsubsection{Residual dipolar couplings}
\begin{quote}
Provides geometric information relative to a common alignment frame.
\end{quote}

\subsubsection{Deduced hydrogen bond}
A hydrogen bond constraint deduced from the data set.

\subsubsection{Torsion angle constraint}
A constraint that describes the orientation between four atoms.


\subsection{CCPN data model}
...
This is called a "DataSource" and corresponds to an NMR spectrum.  It includes
information about the dimensions and spectral meta data.  The definition is
\begin{quote}
This class describes the stored data matrix from an NMR spectrum. 
It is a general class that may be used to describe data in the time domain, 
frequency domain a combination of the two, or axes that are neither time nor 
frequency.  The actual data are kept in a location defined by the dataLocation 
link. The DataSource attributes contain the information required to extract 
the data from the source file, but the data model as such does not include 
the actual data matrix.
\end{quote}

\subsubsection{Peak}
It is defined as 
\begin{quote}Crosspeak at a specific position in a specific spectrum.\end{quote}
according to the documentation.
Peaks have multiple dimensions, one for each dimension of the spectrum in 
which they appear, attributes that apply to the peak as a whole, such as 
intensity, as well as attributes that apply to peak dimensions, such as 
chemical shift.

\subsubsection{Spin system}
Known as "Resonance Group" within the CCPN model, the definition of which is
\begin{quote}
Group of Resonances belonging within the same Residue. Serves to organise 
Resonances that have not yet been assigned, and to set Residue types, 
(sequential) neighbours, Residues and their probabilities for an entire 
group without necesarily assigning the individual Resonances. Coherence 
between ResonanceGroup (probable) assignments and assignments of Resonances 
to Atoms are not checked by the API software. In case of conflicts the 
assignment of the Resonance should take precedence over any information 
carried by the ResonanceGroup, in accordance with the status of the 
ResonanceGroup as a vehicle for tentative assignments.
\end{quote}
ResonanceGroups, along with Resoances, are the main intermediaries between
the molecule and the experimentally observed NMR spectra.

\subsubsection{Resonance}
\begin{quote}
A connection record for assignment. 'Stands for' a single Atom or group of 
atoms in fast exchange. 
The Resonance holds a central place in the Nmr package, as almost all 
assignment, of Chemical shifts, Atoms, Constraints, Peaks etc. is done 
through the intermediary of a Resonance. This allows the relationship 
between Shifts, Peaks, Constraints, (...) to be described precisely without 
first assigning the Nmr spectrum, provides a simple mechanism for handling 
prochiral groups, and in general permits greater flexibility.
\end{quote}
A Resonance is an intermediary between experimental NMR data the molecule
of interest.

\subsubsection{Isotope}
In order to be visible in an NMR experiment, a sample must contain NMR-active
nuclei.  The CCPN model includes information about various NMR-active nuclei,
which are linked to resonances during the process of assignment.

\subsubsection{Molecule, residue and atom}
These are given standard definitions: a protein molecule is a sequence of
residues, each of which is an instance of an amino acid type such Asparagine
or Glycine, and are composed of several atoms.

\subsubsection{Dihedral angle constraint}
\begin{quote}
Must be assigned to a specific quartet of Resonances. 
The different Dihedral Constraint Items represent alternative possibilities for 
dihedral angle values, rather than alternative assignemnts.
\end{quote}

\subsubsection{Hydrogen bond}
\begin{quote}
Constraint (distance constraint) describing a hydrogen bond.
\end{quote}

\subsubsection{Residual dipolar coupling}
An alignment between two nuclei which gives an orientation constraint.

\subsubsection{Violation}
A problem with a structure such that constraint is not satisfied.

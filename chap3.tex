...
and bond angles (see Table-\ref{alanine_bond_angles}) are known \cite{alanine_sreepad}.
...
and concentration are known before running any NMR experiments.  
A sample is often obtained in such a way that it has a specific labeling
pattern of its H, N, and C atoms.

In other NMR applications, such as small molecule metabolite 
identification, the sample is an unknown.

\subsection{NMR}
\subsubsection{Known}
Information about the spectrometer, including frequency, as well as 
gyromagnetic ratios of nuclei (see Table-\ref{gyromagnetic_ratios}) and 
pulse sequences.  The pulse sequences
...
The concept of a resonance is known: that the resonance frequency of an atom is 
...
and width attributes (see Figure-\ref{peak_1d}).

\subsection{Analysis}
The analysis knowns before beginning the process are covered in the
later section describing the library of deductive reasoning.
This section will cover the unknowns.
...
spectra as peak dimensions, and atoms is not known.  The correspondence 
between resonances and peak dimensions is also not known, and is difficult 
to determine in the presence of ambiguity.

These definitions are based on the BMRB and CCPN data models \cite{bmrb, ccpn},
...
\subsubsection{Generic spin system}
A generic spin system is a group of covalently bound resonances.  It may be 
assigned to a protein residue.  It may also be assigned an amino acid type.  
It is a key intermediary between NMR peaks and the protein.
...
(see Figure-\ref{structure_restraints}), residual dipolar coplings (RDCs) 
...
\begin{figure}[h]
  ...
  \caption[A 1-dimensional cross-section of Gaussian peaks]
          {A 1-dimensional cross-section of Gaussian peaks.
           Each peak dimension may be characterized by its 
